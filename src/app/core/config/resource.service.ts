...
export abstract class ResourceService<T> {
  ...
  addAttachment(picture: string, fileExtensions: string, id: number): Observable<any> {
    return this.httpClient
      .post(`${this.APIUrl}/${id}/document`, { content: `data:image/${fileExtensions};base64,${btoa(picture)}` })
      .pipe(
      catchError(err => {
        throw new Error(err.message);
      })
